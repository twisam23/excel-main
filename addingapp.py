...
class StoreApp:
    ...
    def handle_action(self, action, quantity):
        ...
        #
        self.wBook = load_workbook(filename=self.log_file_path)
        self.ws = self.wBook.active  # or specify a particular sheet by name: self.wb['SheetName']
        #
        ...
        if action == "add":
            ...
            if hasattr(self, 'sheet'):
                ...
                #
                self.ws.append([current_time, selected_material, material_code, quantity, "Added"])
                self.wBook.save(self.log_file_path)
                #
            ...
            messagebox.showinfo("Data Submitted", f"Material: {selected_material}\nQuantity: {quantity}\nhas been added to the store.")
                ...
                if hasattr(self, 'sheet'):
                    ...
                    #
                    self.ws.append([current_time, selected_material, material_code, quantity, "Removed"])
                    self.wBook.save(self.log_file_path)
                    #
